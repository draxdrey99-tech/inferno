...
1. **Lead notifications.** As of 10 September 2026 the production database exists (Neon, provisioned through the Vercel Marketplace, tables migrated) and form submissions land in the `leads` table, but nothing tells you. Wire Resend (three lines in `app/api/lead/route.ts`) or push each lead into your own Klaviyo list via their API, which is arguably where an email agency's leads belong. Until this is done the form is a leak.
