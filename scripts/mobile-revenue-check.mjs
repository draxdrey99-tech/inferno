...
for(const width of [320,375,768]){
 ...
 for(let i=0;i<3;i++){
  ...
  await page.waitForTimeout(1900);checks.push({width,scene:i,check:'exact final digits',pass:await reels.locator('.revenue-strip').evaluateAll(es=>es.every(e=>Math.abs(-new DOMMatrix(getComputedStyle(e).transform).m42/(e.getBoundingClientRect().height/10)-Number(e.dataset.digit))<.01&&e.getAnimations().length===0))});
...
await browser.close();await fs.writeFile('docs/verification/revenue-reels.json',JSON.stringify(checks,null,2));console.log({checks:checks.length,failures:checks.filter(c=>!c.pass)});if(checks.some(c=>!c.pass))process.exitCode=1;
