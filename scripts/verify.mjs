...
await page.locator('#name').fill('Test');await page.locator('#email').fill('test@example.test');await page.getByRole('button',{name:'Send my details'}).click();await page.locator('#audit-form [role="alert"]').waitFor();check('form error feedback',await page.locator('#audit-form [role="alert"]').isVisible());
await page.unroute('**/api/lead');await page.route('**/api/lead',route=>route.fulfill({status:200,contentType:'application/json',body:'{"ok":true}'}));await page.getByRole('button',{name:'Send my details'}).click();await page.getByRole('status').waitFor();check('form confirmation',await page.getByRole('status').isVisible());
