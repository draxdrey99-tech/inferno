 ...
 return <section id="proof" className="proof-section" aria-labelledby="proof-title"><div className="shell">
  ...
  {PROOF.map((p,i)=><article className="proof-scene" key={p.image} aria-label={p.headline}>
   <div className="proof-panel">
    <div className="proof-readout"><p className="proof-window">{p.window}</p><p className="proof-amount"><span className="sr-only">{new Intl.NumberFormat('en-US',{style:'currency',currency:i===2?'GBP':'USD'}).format(amounts[i])}</span><span aria-hidden="true" data-revenue={amounts[i]} data-currency={i===2?'GBP':'USD'}>{new Intl.NumberFormat('en-US',{style:'currency',currency:i===2?'GBP':'USD'}).format(amounts[i])}</span></p><h3>{p.headline}</h3><p className="proof-share">{p.metric}</p><p className="proof-note">{p.note}</p></div>
