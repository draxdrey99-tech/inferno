...
export default function Hero(){
 return <section id="hero" aria-labelledby="hero-title" className="hero">
  <div className="shell hero-grid">
   <div className="hero-copy">
    <p className="eyebrow hero-desktop-only">Email marketing for DTC &amp; ecommerce brands</p>
    <p className="eyebrow hero-mobile-only">Klaviyo for ecommerce brands</p>
    <h1 id="hero-title" className="display-xl">{headline.split(' ').map((word,i)=><span className="hero-mask" key={i}><span className="hero-word">{word}</span>{' '}</span>)}</h1>
    <p className="lede hero-subhead hero-desktop-only">Klaviyo flows, campaigns designed in your brand, and the deliverability work underneath so it actually reaches the inbox.</p>
  ...
  <details className="shell hero-mobile-work hero-mobile-only">
   <summary>See client work <span aria-hidden="true">+</span></summary>
   <div className="hero-mobile-work-content">
    ...
    <p className="hero-mobile-clients">Separate account result: $477,929.14 attributed to email · 4 Sep - 4 Nov 2023. Unattributed account; client creative shown separately above.</p>
    ...
    ].map(([value,label])=><p key={label}><strong>{value}</strong> {label}</p>)}</div>
    <Marquee/>
   </div>
  </details>
  ...
  ].map(s=><div key={s.label}><strong>{s.value}</strong><p>{s.label}</p></div>)}</div>
  <div className="border-y border-white/15 hero-desktop-only"><Marquee/></div>
 </section>;
}
