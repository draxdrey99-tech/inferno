/** Native, one-shot number reels for touch and small screens. No scroll library. */
export function mountMobileRevenue() {
  ...
  const observer = new IntersectionObserver(entries => {
    for (const entry of entries) {
      ...
      entry.target.querySelectorAll<HTMLElement>('.revenue-strip').forEach((strip, index) => {
        ...
        ], {duration: 1100, delay: index * 20, easing: 'cubic-bezier(.16,1,.3,1)'});
        animations.add(animation);
        void animation.finished.catch(() => {}).finally(() => animations.delete(animation));
      });
    }
  }, {threshold: .65, rootMargin: '0px 0px -8% 0px'});
