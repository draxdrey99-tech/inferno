...
export default async function ServicePage({params}:Props){
 ...
 <section className="shell inner-page" aria-labelledby="service-title"><nav aria-label="Breadcrumb" className="breadcrumb"><Link href="/">Home</Link><span>/</span><Link href="/services">Services</Link><span>/</span><span>{s.title}</span></nav>
 <p className="eyebrow">{s.keyword}</p><h1 id="service-title" className="display-xl mt-5 max-w-4xl">{s.title} for ecommerce brands.</h1><p className="lede mt-7 max-w-3xl">{s.intro}</p><a className="btn btn-flame mt-8" href={SITE.calendly} target="_blank" rel="noopener noreferrer">Book your free audit</a><p className="mt-3 text-sm text-mute">30 minutes, no obligation, and the findings are yours to keep.</p>
